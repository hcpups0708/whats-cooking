...
def main():
    ...
    gnb=RandomForestClassifier(verbose=1,n_jobs=2,min_samples_leaf=1,n_estimators=2,oob_score=1)
    ...
    features={}
    for idx,val in enumerate(gnb.feature_importances_):
        features[featureNames[idx]]=val

    sorted_features = sorted(features.items(), key=operator.itemgetter(1), reverse=True)
    print(sorted_features)
    for i in range(0,20):
        print(sorted_features[i][0],sorted_features[i][1])
