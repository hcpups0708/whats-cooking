...
group_it=['mexican','italian','southern_us']
group_cn=['chinese','french']
group_su=['vietnamese','moroccan','filipino','korean','brazilian','jamaican','russian']
group_ib=['irish','british']
group_mb=['spanish','japanese','greek','cajun_creole','thai']
...
for nameidx,samples in enumerate(Xunb):
	for i in range(0,7837):
		Xbalanced.append(samples[i%len(samples)])
		Ybalanced.append(labels[nameidx])
...
vc=VotingClassifier(estimators=[('etc',etc),('lr', lr), ('knn', knn), ('rf', rf)], voting='soft',weights=[5,4,1,1])
...
for idx,dish in enumerate(X.toarray()):
    if Y[idx] in group_it:
        X_it.append(dish)
        Y_it.append(Y[idx])
    if Y[idx] in group_cn:
        X_cn.append(dish)
        Y_cn.append(Y[idx])
    if Y[idx] in group_su:
        X_su.append(dish)
        Y_su.append(Y[idx])
    if Y[idx] in group_ib:
        X_ib.append(dish)
        Y_ib.append(Y[idx])
    if Y[idx] in group_mb:
        X_mb.append(dish)
        Y_mb.append(Y[idx])
...
if local_test:
	...
	if cm:
		for row in confusion_matrix(Y_test,predictions,labels):
			for n in row:
				print(str(n)+" "),
			print("")
else:
