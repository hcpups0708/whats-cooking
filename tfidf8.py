...
X_all=np.concatenate((tfidftr,X_ing),axis=1)
print("X_all",X_all.shape)
X_unknown_all=np.concatenate((tfidfts,X_unknown_ing),axis=1)
print("X_unknown_all",X_unknown_all.shape)
Y = traindf['cuisine']

#print("tfidftr",tfidftr.shape)
#print("tfidfts",tfidfts.shape)
rf_sel=RandomForestClassifier(verbose=1,n_jobs=20,min_samples_leaf=1,n_estimators=100,oob_score=1,max_features='log2')
svc_sel = SVC(verbose=1,kernel="linear")
selpca_ing=PCA(n_components=3000)
selkb_ing=SelectKBest(f_classif,k=4000)
selp_ing=SelectPercentile(f_classif,percentile=95)

sel=selkb_ing

X = sel.fit_transform(X_all,traindf['cuisine'])
X_unknown=sel.transform(X_unknown_all)
print("X",X.shape)
...
testdf[['id' , 'cuisine' ]].to_csv("submission8.csv",index = False)
